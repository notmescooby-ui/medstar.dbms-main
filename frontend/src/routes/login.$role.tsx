...
function LoginRole() {
  ...
  const backTo = ["receptionist", "sister", "rmo"].includes(roleKey)
    ? "/login/reception"
    : "/login";
  ...
  return (
    <main className="grid min-h-screen grid-cols-1 md:grid-cols-[1.05fr_1fr]">
      ...
      <section className="hidden bg-[color:var(--sidebar)] p-14 text-white md:flex md:flex-col md:justify-between">
        ...
        <div>
          <div className="text-[10px] uppercase tracking-[0.22em] text-white/50">{meta.title}</div>
          <h1 className="serif-display mt-3 text-[46px] leading-tight text-white">{meta.short}</h1>
          <p className="mt-4 max-w-md text-[15px] leading-relaxed text-white/70">{meta.blurb}</p>
        </div>
        <div className="text-xs text-white/40">
          MEDSTAR Multispeciality Hospital · Internal Portal
        </div>
      </section>
      ...
      <section className="flex flex-col bg-background px-6 py-10 md:px-14 md:py-16">
        <div className="mb-8 flex items-center justify-between md:hidden">
          <BrandLogo size="sm" />
          <Link to={backTo} className="text-xs text-muted-foreground">
            ← Back
          </Link>
        </div>
        <div className="mx-auto flex w-full max-w-md flex-1 flex-col justify-center">
          ...
          <form onSubmit={onSubmit} className="mt-10 space-y-5">
            ...
            {error && (
              <div className="rounded-md border border-[color:var(--destructive)]/30 bg-[color:var(--destructive)]/5 px-3 py-2 text-xs text-[color:var(--destructive)]">
                {error}
              </div>
            )}
            ...
            <button
              type="submit"
              className="w-full rounded-md bg-[color:var(--sidebar)] py-3 text-sm font-medium text-white transition hover:bg-[oklch(0.22_0.012_45)]"
            >
              Log in
            </button>

            <div className="pt-2 text-[11px] text-muted-foreground">
              Demo: any active employee name for this role (e.g.{" "}
              <code>
                {roleKey === "director"
                  ? "Dr. Arvind Rao"
                  : roleKey === "tpa"
              ...
              ) with password <code>medstar</code>.
            </div>
          </form>
        </div>
      </section>
