...
import { summary, notices, auditTrail, calendarEvents, handoverNotes, patients, indents, dutyRoster, claims, queries, rooms } from "@/lib/mock-data";
...
function tileGrid<T>(items: T[], render: (t: T) => React.ReactNode) {
  return <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">{items.map(render)}</div>;
}
...
export function DirectorDashboard({ user }: { user: SessionUser }) {
  const tiles = [
    ...
    { label: "Occupied beds", value: summary.occupiedBeds, hint: `${summary.vacantBeds} vacant across floors` },
    { label: "Vacant beds", value: summary.vacantBeds, accent: "sage" as const, hint: "Ground · First · Second" },
    { label: "Pending insurance claims", value: summary.pendingClaims, accent: "gold" as const, hint: "4 documents requested" },
    { label: "Outstanding bills", value: summary.outstandingBills, hint: "Across 9 patient accounts" },
    { label: "Employees on duty", value: summary.employeesOnDuty, hint: "Morning shift · all departments" },
    { label: "Medical camps today", value: summary.medicalCampsToday, hint: "Community atrium — 9 AM to 2 PM" },
  ...
  return (
    <>
      ...
      <Section title="At a glance" description="Numbers only — dedicated pages for detail.">
        {tileGrid(tiles, (t) => <SummaryTile key={t.label} {...t} />)}
      </Section>

      <Section title="Recent audit trail" description="Every consequential action across the hospital.">
        <DataTable
          columns={[
            { key: "at", header: "When", className: "w-40 text-muted-foreground" },
            { key: "user", header: "Who" },
            { key: "role", header: "Role", render: (r) => <Chip label={ROLE_META[r.role].title} tone="sage" /> },
          ...
          {handoverNotes.slice(0, 2).map((h) => (
            <div key={h.id} className="rounded-lg border border-hairline bg-card p-6">
              <div className="flex items-center justify-between">
                <div className="eyebrow">{h.department} · {h.shift}</div>
...
export function ReceptionistDashboard({ user }: { user: SessionUser }) {
  ...
  return (
    <>
      ...
      <Section title="Front desk">{tileGrid(tiles, (t) => <SummaryTile key={t.label} {...t} />)}</Section>
      ...
      <Section title="Quick actions" description="Common receptionist workflows.">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {["New admission", "Assign room", "Log payment", "Add handover"].map((a) => (
            <button key={a} className="rounded-lg border border-hairline bg-card p-5 text-left transition hover:shadow-[var(--shadow-quiet)]">
      ...
      <Section title="Upcoming events" description="From the hospital calendar.">
        <div className="rounded-lg border border-hairline bg-card">
          {calendarEvents.slice(0, 5).map((e, i) => (
            <div key={e.id} className={"flex items-center gap-4 px-6 py-4 " + (i > 0 ? "border-t border-hairline" : "")}>
              <span className={"h-2.5 w-2.5 rounded-full " + kindDot(e.kind)} />
              <div className="flex-1">
                <div className="text-[14px] font-medium">{e.title}</div>
                <div className="text-xs text-muted-foreground">{e.location} · {e.time}</div>
              </div>
...
function kindDot(k: string) {
  ...
  } as Record<string, string>)[k] ?? "bg-muted-foreground";
}
export { kindDot };
...
export function SisterDashboard({ user }: { user: SessionUser }) {
  ...
  return (
    <>
      ...
      <Section title="Your shift">{tileGrid(tiles, (t) => <SummaryTile key={t.label} {...t} />)}</Section>
...
export function RmoDashboard({ user }: { user: SessionUser }) {
  ...
  return (
    <>
      ...
      <Section title="Clinical floor">{tileGrid(tiles, (t) => <SummaryTile key={t.label} {...t} />)}</Section>
...
export function TpaDashboard({ user }: { user: SessionUser }) {
  const tiles = [
    { label: "Pending claims", value: claims.filter(c => c.status === "Pending").length, accent: "gold" as const },
  ...
  return (
    <>
      ...
      <Section title="Insurance desk">{tileGrid(tiles, (t) => <SummaryTile key={t.label} {...t} />)}</Section>
...
export function claimTone(s: string) {
  return ({
    ...
    "Query Raised": "info",
  } as const)[s as "Approved"] ?? "neutral";
}
